...
from .utils import (
    ...
    parse_simple_yaml_to_obj as _parse_simple_yaml_to_obj,
    canonical_json_bytes as _canonical_json_bytes,
    cloud_post_json as _cloud_post_json,
    cloud_get_json as _cloud_get_json,
    render_email_preview as _render_email_preview,
    email_config_to_yaml as _email_config_to_yaml,
)
...
def create_app() -> Flask:
    ...
    def results(campaign_key: str) -> str:
        cloud_base_url, cloud_admin_token = _get_cloud_config()
        with db.connect() as conn:
            campaign = get_campaign_by_key(conn, campaign_key=campaign_key)
            if campaign is None:
                flash("Campaign not found", "error")
    ...
    def cloud_tokens_csv(campaign_key: str) -> Response:
        cloud_base_url, _ = _get_cloud_config()
        with db.connect() as conn:
            campaign = get_campaign_by_key(conn, campaign_key=campaign_key)
            if campaign is None:
                return Response("Campaign not found", status=404)
